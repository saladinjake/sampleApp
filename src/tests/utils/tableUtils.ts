  /* eslint-ignore testing-library/no-node-access  */
...
const getClosestTable = (cell: HTMLElement) => {
  ...
  return cell.closest(    /* eslint-ignore testing-library/no-node-access  */
...
const getHeaders = (cell: HTMLElement) => {
  ...
  const headers = (cell as HTMLTableCellElement).headers
    .split(" ")
    .filter(Boolean)
    .map((headerId) => table.querySelector(`#${headerId}`))   /* eslint-ignore testing-library/no-node-access  */
...
const queryAllByHeader = (
  ...
  const relevantCellsByHeadersAttribute = header.id
    ? Array.from(table.querySelectorAll("[headers]")).filter((cell) => {   /* eslint-ignore testing-library/no-node-access  */
